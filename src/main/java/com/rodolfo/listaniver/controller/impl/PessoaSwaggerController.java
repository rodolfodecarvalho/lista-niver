...
public interface PessoaSwaggerController {

    @Operation(summary = "Criar nova pessoa", description = "Cria uma nova pessoa no sistema")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Pessoa criada com sucesso",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = PessoaOutputDTO.class))),
            @ApiResponse(responseCode = "400", description = "Data inválida",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = ProblemDetail.class))),
            @ApiResponse(responseCode = "422", description = "Dados inválidos",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = ProblemDetail.class))),
            @ApiResponse(responseCode = "409", description = "Pessoa já existe",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = ProblemDetail.class)))
    })
    @PostMapping
    ...
    @GetMapping("/{id}")
    ResponseEntity<PessoaOutputDTO> buscarPorId(
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "404", description = "Pessoa não encontrada",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = ProblemDetail.class))),
            @ApiResponse(responseCode = "409", description = "Pessoa com dados duplicados já existe",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = ProblemDetail.class)))
    })
    @PutMapping("/{id}")
    ResponseEntity<PessoaOutputDTO> atualizar(
            @Parameter(description = "ID da pessoa", required = true, example = "1")
            @PathVariable Long id,
    ...
    @GetMapping("/buscar")
    ResponseEntity<List<PessoaOutputDTO>> buscarPorNome(
            @Parameter(description = "Nome ou parte do nome da pessoa", required = true, example = "João")
